...
#...................Data.....................
Nc_data = {'Si':2.8e25, 'Ge':1.04e25,'GaAs':4.7e23,'AlGaAs':7.47e23,'InP':5.7e23,'GaInAs':2.1e23 }  # m^-3
Nv_data = {'Si':1.04e25, 'Ge':6.0e24,'GaAs':7.0e24,'AlGaAs':1.120e25,'InP':1.1e25,'GaInAs':7.71e24}  # m^-3
Eg_Data = {'Si':1.12, 'Ge':0.66,'GaAs':1.43,'AlGaAs':1.923,'InP':1.334,'GaInAs':0.74 }  # eV
#EA_Data= {'Si':4.05,  'Ge':4.0}  # eV
epsilon_Data = {'Si':11.7,'Ge':16.0,'GaAs':12.9,'AlGaAs':11.764,'InP':12.5,'GaInAs':12.5} 
#Length_data={500e-9,300e-9,500e-9}
...
for d in range(domain):
    ...
    len =float(input("Enter the length of the material in m: "))
    Length.append(len)
...
# Assuming electric_field should be constant at the interface

for i in range(0,domain-1):
    electric_field[nodes[i]]=electric_field[nodes[i]-1]
    nextnode=nodes[i]+nodes[i+1]
    electric_field[nextnode]=electric_field[nextnode+1]
...
plt.figure(figsize=(10, 6))
plt.plot(x_values, rhos[1500:3000],label='Charge', color='magenta')
